...
while true
  ...
  currentReading = struct ('start_angle', 0,'angular_resolution', 0,'maximum_range', 0,'ranges', [],'pose', zeros(3,1),'laser_offset', zeros(3,1),'timestamp', 0);
  tk = 1;
  currentReading.start_angle = num_tokens(tk+2);
  tk = tk + 3; % skip FOV
  currentReading.angular_resolution = num_tokens(tk+1);
  currentReading.maximum_range = num_tokens(tk+2);
  tk = tk + 5 ; % skip accuracy, remission_mode

  num_readings = int32(num_tokens(tk));
  tk = tk + 1;
  currentReading.ranges = num_tokens(tk:tk+num_readings-1);
  tk = tk + num_readings-2;
...
%  tk = tk+ num_remissions;
  ...
  currentReading.laser_offset = num_tokens(tk:tk+2);
  %tk =tk+3;

  tk =tk+ 3; % skip tv, rv, forward, side, turn
  currentReading.timestamp = num_tokens(tk+2);
