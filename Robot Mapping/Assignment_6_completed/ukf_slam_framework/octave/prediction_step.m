...
mu = zeros(n,1); % re-initialize based on the current n size 
sigma = zeros(n);
...
% cosines and recover the 'average' angle via atan2)
    ...
    x_bar =  xbar + wm(i)*cos(sigma_points(3,i));
    y_bar =  ybar + wm(i)*sin(sigma_points(3,i));
    endfor  
    mu(3,1) = normalize_angle(atan2(y_bar,x_bar));
