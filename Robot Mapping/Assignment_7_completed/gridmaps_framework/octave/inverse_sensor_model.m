function [mapUpdate, robPoseMapFrame, laserEndPntsMapFrame] = inverse_sensor_model(map, scan, robPose, gridSize, offset, probOcc, probFree)
...
robPoseMapFrame(1:2,1) = world_to_map_coordinates(robPose, gridSize, offset); 
robPoseMapFrame(3,1) = robPose(3,1);
...
for sc1=1:size(laserEndPntsMapFrame,2)
%TODO: compute the XY map coordinates of the free cells along the laser beam ending in laserEndPntsMapFrame(:,sc)
    if((laserEndPnts(1,sc1)~=0)&&(laserEndPnts(2,sc1)~=0))
    [X,Y] = bresenham([robPoseMapFrame(1), robPoseMapFrame(2); laserEndPntsMapFrame(1,sc1), laserEndPntsMapFrame(2,sc1)]);
...
for i=1:size(laserEndPntsMapFrame,2)
    if((laserEndPnts(1,i)~=0)||(laserEndPnts(2,i)~=0))
    mapUpdate(floor(laserEndPntsMapFrame(1,i)),floor(laserEndPntsMapFrame(2,i))) = prob_to_log_odds(probOcc);
    end
end
