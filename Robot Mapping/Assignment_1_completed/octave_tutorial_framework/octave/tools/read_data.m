function data = read_data(filename)
    ...
    odom = struct;
    sensor(size(input))= struct('id', cell(size(input)),'range' ,cell(size(input)),'bearing' , cell(size(input)));
    ...
    while(~feof(input))
        ...
        if(strcmp(type, 'ODOMETRY') == 1)
            if(first == 0)
                ...
                odom = struct;
                sensor = struct('id', cell(size(input)),'range' ,cell(size(input)),'bearing' , cell(size(input)));
            end
            first = 0;
        ...
        elseif(strcmp(type, 'SENSOR') == 1)
            reading = struct('id', cell(size(1)),'range' ,cell(size(1)),'bearing' , cell(size(1)));
