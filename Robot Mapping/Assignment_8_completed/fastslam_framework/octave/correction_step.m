...
for i = 1:numParticles
  ...
  for j = 1:m
    ...
    if (particles(i).landmarks(l).observed == false)
      ...
      % TODO: Initialize landmark's position based on the measurement and the current robot pose:
       particles(i).landmarks(l).mu(1,1) = robot(1)+ (z(j).range*cos(robot(3)+ z(j).bearing));
       particles(i).landmarks(l).mu(2,1) = robot(2)+ (z(j).range*sin(robot(3)+ z(j).bearing));   % 2D position of the landmark
      ...
      K = particles(i).landmarks(l).sigma*H'*(Q)^(-1);
      
      % TODO: compute the error between the z and expectedZ (remember to normalize the angle)
      observe_error(1,1) =  z(j).range - expectedZ(1);
      observe_error(2,1)= normalize_angle(z(j).bearing-expectedZ(2));
      % TODO: update the mean and covariance of the EKF for this landmark
      particles(i).landmarks(l).mu = particles(i).landmarks(l).mu + K*observe_error;
      ...
      particles(i).weight = particles(i).weight + (1/sqrt(det((2*3.14.*Q))))*(exp(-0.5*(observe_error)'*Q^(-1)*observe_error));
